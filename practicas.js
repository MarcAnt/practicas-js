...
const mimi = new OtroAnimal('Mimi', 'Hembra'), scooby = new OtroPerro('Scooby', 'Macho', 'Gigante'); 
...
function countChart(words = '') {
	...
	if (typeof typeEntry == 'number' || typeof typeEntry == 'boolean' || typeof typeEntry == 'function' 
		|| typeof typeEntry == 'object' || typeof typeEntry == 'array' || typeof typeEntry == null 
		|| typeof typeEntry == undefined || !typeEntry) {
...
const cortarTexto = (text = '') => 
	(!text || typeof text == 'number' || typeof text == 'boolean' || typeof text == 'function' 
		|| typeof text == 'object' || typeof text == 'array' || typeof text == null 
...
const repeatWord = (words = '', repeat = undefined) => {
	...
	if(repeat === undefined) return console.warn('No ingresaste el numero de veces'); 
    if(repeat <= 0) return console.error('No puedes ingresar valores negativos o cero ');
...
const palindrome = (palind = '') => {
	...
	? console.info(`Si es palíndromo. La palabra original es ${palind}, palabra al revés ${alReves}`)
	: console.info(`No es palíndromo. La palabra original es ${palind}, palabra al revés ${alReves}`)
}
...
const convertidor = (unidad = 'C', temperatura = 37) => {
	...
	if (typeof unidad !== 'string') return console.warn('Debes insertar una unidad válida. pe: "C" o "F"')
	if (unidad.toLowerCase() !== 'c' && unidad.toLowerCase() !== 'f' ) return console.error('No es la unidad correcta')
	...
	if(unidad.toLowerCase() === 'c') {
		...
		return console.info(`La temperatura de ${temperatura} °F es ${result.toFixed(2)} °C`)
	}
	...
	if(unidad.toLowerCase() === 'f') {
		...
		return console.info(`La temperatura de ${temperatura} °C es ${result.toFixed(2)} °F`)
...
const miDescuento = (cantidad = undefined, descuento = 0) => {
	...
	if(cantidad === undefined || descuento === undefined) return console.warn('No ingresaste un número');
	if(typeof cantidad !== 'number'|| typeof descuento !== 'number') return console.error('El valor ingresado no es un número');
	if(cantidad === 0 ) return console.error('El valor no puede ser 0');
	if(Math.sign(cantidad) === -1 || Math.sign(descuento) === -1) return console.error('El número no puede ser negativo');


	return console.info(`Tu monto inicial es ${cantidad} y tu descuento aplicado de ${descuento} es ${cantidad - ((descuento * cantidad) / 100)}` ); 
...
const calcularEdad = (fecha = undefined) => {
	...
	let hoyMenosFecha = new Date().getTime() - fecha.getTime(), yearsEnMS = 1000 * 60 * 60 * 24 * 365,
	 yearHumanos = Math.floor(hoyMenosFecha / yearsEnMS); 

	return (Math.sign(yearHumanos) === -1 )
		? console.info(`Faltan ${Math.abs(yearHumanos)} años para el ${fecha.getFullYear()}`)
		:(Math.sign(yearHumanos) === 1)
			? console.info(`Han pasado ${Math.abs(yearHumanos)} años desde ${fecha.getFullYear()}`)
...
const contarVocalesConsonantes = (texto = '') => {
	...
	if (typeof texto == 'number' || typeof texto == 'boolean' || typeof texto == 'function' 
		|| typeof texto == 'object' || typeof texto == 'array' || typeof texto == null 
	...
	return console.info(`Vocales: ${contarVocales} Consonantes: ${contarConsonantes}`); 
...
const potencias = ( potenciales = undefined) => {
	...
	if(!potenciales instanceof Array) return console.warn('El tipo de dato no es un array'); 
	if(potenciales.lenght === 0) return console.error('El array no debe estar vacio'); 
	if(potenciales === undefined) return console.error('Ingresa un arreglo con números'); 


	for (let num of potenciales) {
		if(typeof num !== 'number') return console.error('Ingreses solo valores que sean números'); 
...
const maxMin = ( numeros = undefined) => {
	...
	if(!numeros instanceof Array) return console.warn('El tipo de dato no es un array'); 
	if(numeros.lenght === 0) return console.error('El array no debe estar vacio'); 
	if(numeros === undefined) return console.error('Ingresa un arreglo con números'); 
...
// 23) Programa una función que dado un array de números devuelva un objeto con 2 arreglos en el primero almacena los
 ...
 const objParImpar = (valores = undefined) => {
	...
	if(!valores instanceof Array) return console.warn('El tipo de dato no es un array'); 
	if(valores.lenght === 0) return console.error('El array no debe estar vacio'); 
	if(valores === undefined) return console.error('Ingresa un arreglo con números'); 
	...
	valores.filter( el => (el % 2 === 0) ? obj.pares.push(el) : obj.impares.push(el)  )
 ...
 const ordenar = (arr = undefined)  => {
 	...
 	if(!arr instanceof Array) return console.warn('El tipo de dato no es un array'); 
...
const quitarDuplicados = (arr = undefined) => {
	...
	if(!arr instanceof Array) return console.warn('El tipo de dato no es un array'); 
	if(arr.lenght === 0) return console.error('El array no debe estar vacio'); 
	if(arr.lenght === 1) return console.error('El deben haber al menos dos elementos para poder comparar'); 
	if(arr === undefined) return console.error('Ingresa un arreglo con números'); 
	...
	return console.info(
		...
		arr.reduce( (total, num, index, arr) => {
			...
			if (index === arr.length - 1) { 
				return `El primedio de datos es: ${ arr.join(' + ') } es ${total/arr.length}`; 
			}else {
...
class Pelicula {
	...
	static get listaGeneros() {
		return ['Action', 'Adult', 'Adventure', 'Animation', 'Biography', 
				'Comedy', 'Crime', 'Documentary' ,'Drama', 'Family', 'Fantasy', 'Film Noir', 'Game-Show',
				'History', 'Horror', 'Musical', 'Music', 'Mystery', 'News', 'Reality-TV', 'Romance', 'Sci-Fi', 'Short', 
	...
	validarArreglo(propiedad, valor) {
		...
		if(!valor instanceof Array) return console.warn('El tipo de dato no es un array'); 
		if(valor.lenght === 0) return console.error('El array no debe estar vacio'); 
		if(!valor) return console.error('Ingresa un arreglo con números'); 

		for (let cadena of valor) {
			if(typeof cadena !== 'string') return console.error(`El valor de la ${cadena} ingresado, no es una cadena de texto`); 
			// if(num === 0) return console.error('Ingreses valores mayores a 0'); 
		}
	...
	validarIMDB(id) {
		...
		if (this.validarCadena('IMDB id', id)){
			...
			if (!/^([a-z]){2}([0-9]){7}$/.test(id)) return console.error(`El IMDB id "${id}" no debe tener 9 caracteres, los 2 primeros letras 
	...
	validarEstreno(estreno) {
		...
		if (this.validarNumero('Año de Estreno', estreno)){
			...
			if (!/^([0-9]){4}$/.test(estreno)) return console.error(`El año de estreno "${estreno}" no es válido,
	...
	validarCalificacion(calificacion) {
		...
		if (this.validarNumero('Calificacion', calificacion)){
			...
			return (calificacion < 0 || calificacion > 10)
				? console.error(`La calificación debe de estar en un rango entre 0 y 10`)
...
//Callbacks Una forma de controlar los llamados asíncronos 
 ...
 // 	.catch(err => console.error(err)); 
	...
	wm.set(llave2, 2); 
 	...
 	setTimeout(() => {llave1 = null; llave2 = null; llave3 = null},5000);
